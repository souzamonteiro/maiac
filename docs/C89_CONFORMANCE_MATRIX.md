...
| Storage/type qualifiers | storageClassSpecifier, typeQualifier | done | done | done | done | Tier 2 | Phase 12 validates runtime behavior for const/volatile/register/auto/static and typedef-with-qualifier forms; static local storage now preserves state across calls; extern declaration + same-unit definition resolves correctly; unresolved extern variable declarations currently use explicit zero fallback behavior; writes through pointer-to-const are rejected; duplicate `const` qualifiers are diagnosed. |
