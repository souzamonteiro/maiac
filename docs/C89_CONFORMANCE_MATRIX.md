...
| Builtin and named types | builtinTypeSpecifier, namedTypeSpecifier, typedefName | done | partial | partial | partial | Tier 2 | Phase 13 validates char/short/long/float/double/signed/unsigned basics, typedef chains, enum/struct named-type paths, forward struct tags, and rejection of local variables declared as `void`; unsigned comparison/signedness semantics remain partial. |
