...
const cliFolder = getArgValue('--folder') || process.env.SERVE_FOLDER;
const cliPort = getArgValue('--port') || process.env.PORT;
const cliSslPort = getArgValue('--ssl-port') || process.env.SSL_PORT;
const cliSslKey = getArgValue('--ssl-key') || process.env.SSL_KEY;
const cliSslCert = getArgValue('--ssl-cert') || process.env.SSL_CERT;

if (cliPort && !Number.isNaN(Number(cliPort))) {
    config.port = Number(cliPort);
}
if (cliSslPort && !Number.isNaN(Number(cliSslPort))) {
    config.sslPort = Number(cliSslPort);
}
if (cliFolder) {
    config.folder = cliFolder;
}
if (cliSslKey) {
    config.sslKey = cliSslKey;
}
if (cliSslCert) {
    config.sslCert = cliSslCert;
}
if (hasArg('--no-ssl') || process.env.NO_SSL === '1') {
    config.enableSSL = false;
}

config.folder = resolvePathMaybe(BASE_DIR, config.folder);
config.sslKey = resolvePathMaybe(BASE_DIR, config.sslKey);
config.sslCert = resolvePathMaybe(BASE_DIR, config.sslCert);

if (config.enableSSL && (!fs.existsSync(config.sslKey) || !fs.existsSync(config.sslCert))) {
    const fallbackKey = path.resolve(BASE_DIR, 'localhost-key.pem');
    const fallbackCert = path.resolve(BASE_DIR, 'localhost.pem');
    if (fs.existsSync(fallbackKey) && fs.existsSync(fallbackCert)) {
        config.sslKey = fallbackKey;
        config.sslCert = fallbackCert;
    }
}
...
function serveFile(req, res, filePath) {
    ...
    fs.readFile(filePath, (err, content) => {
        if (err) {
            ...
            // Cache headers
            if (config.cache) {
                // Keep test artifacts fresh in browser: disable caching for app/runtime files.
                if (ext === '.html' || ext === '.htm' || ext === '.js' || ext === '.wasm' || ext === '.json' || ext === '.map') {
...
function requestHandler(req, res) {
    ...
    if (pathname === '/favicon.ico') {
        const iconPath = path.join(config.folder, 'favicon.ico');
    ...
    if (pathname === '/') {
        const indexPath = path.join(config.folder, 'index.html');
        const browserRunnerPath = path.join(config.folder, 'browser-runner.html');
        if (fs.existsSync(indexPath)) {
            filePath = indexPath;
        } else if (fs.existsSync(browserRunnerPath)) {
            filePath = browserRunnerPath;
        } else {
            filePath = indexPath;
        }
    } else {
