;; Generated by MaiaC
;; Template backend: human-readable WAT
;; Style inspired by maiawasm/assembler/tests/fixtures

(module
  (import "env" "printf" (func $imp_printf (param f64 f64 f64 f64 f64 f64 f64 f64) (result i32)))

  (memory $mem 1)

  (table $fn_table 2 funcref)

  ;; global __frame_ptr
  (global $__frame_ptr (mut i32) (i32.const 0))

  ;; global __stack_ptr
  (global $__stack_ptr (mut i32) (i32.const 1024))

  ;; global ERR_OK
  (global $ERR_OK i32 (i32.const 0))

  ;; global ERR_INT
  (global $ERR_INT i32 (i32.const 1))

  ;; global ERR_STRING
  (global $ERR_STRING i32 (i32.const 2))

  ;; global stdin
  (global $stdin (mut i32) (i32.const 0))

  ;; global stdout
  (global $stdout (mut i32) (i32.const 0))

  ;; global stderr
  (global $stderr (mut i32) (i32.const 0))

  (data (i32.const 16) "Oops!\00")
  (data (i32.const 24) "\02\00\00\00")
  (data (i32.const 28) "\01\00\00\00")
  (data (i32.const 32) "An error occurred: %d.\0a\00")
  (data (i32.const 56) "An error occurred: %s.\0a\00")

  (elem (table $fn_table) (i32.const 0) func $do_work $main)

  ;; function do_work
  (func $do_work (param $trigger_string_error i32) (param $int_error_value i32) (param $msg i32) (param $msg_size i32) (result i32)
    (local $__frame i32)
    (local $i i32)
    (local $src i32)
    (local $__tmp_i32 i32)
    (local $__tmp_i8 i32)
    (local $__tmp_addr i32)
    (local $__tmp_old_i32 i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 24
    i32.add
    global.set $__stack_ptr
    local.get $__frame
    i32.const 0
    i32.add
    local.get $trigger_string_error
    i32.store
    local.get $__frame
    i32.const 4
    i32.add
    local.get $int_error_value
    i32.store
    local.get $__frame
    i32.const 8
    i32.add
    local.get $msg
    i32.store
    local.get $__frame
    i32.const 12
    i32.add
    local.get $msg_size
    i32.store
    local.get $__frame
    i32.const 0
    i32.add
    i32.load
    i32.eqz
    i32.eqz
    if
      local.get $__frame
      i32.const 20
      i32.add
      i32.const 16
      i32.store
      local.get $__frame
      i32.const 16
      i32.add
      i32.const 0
      local.set $__tmp_i32
      local.get $__tmp_i32
      i32.store
      local.get $__tmp_i32
      drop
      block $do_work_for_exit_0
        loop $do_work_for_loop_2
          local.get $__frame
          i32.const 16
          i32.add
          i32.load
          local.get $__frame
          i32.const 12
          i32.add
          i32.load
          i32.const 1
          i32.sub
          i32.lt_s
          i32.eqz
          i32.eqz
          if (result i32)
            local.get $__frame
            i32.const 20
            i32.add
            i32.load
            local.get $__frame
            i32.const 16
            i32.add
            i32.load
            i32.const 1
            i32.mul
            i32.add
            i32.load8_u
            i32.eqz
            i32.eqz
          else
            i32.const 0
          end
          i32.eqz
          i32.eqz
          i32.eqz
          br_if $do_work_for_exit_0
          block $do_work_for_continue_1
            local.get $__frame
            i32.const 8
            i32.add
            i32.load
            local.get $__frame
            i32.const 16
            i32.add
            i32.load
            i32.const 1
            i32.mul
            i32.add
            local.get $__frame
            i32.const 20
            i32.add
            i32.load
            local.get $__frame
            i32.const 16
            i32.add
            i32.load
            i32.const 1
            i32.mul
            i32.add
            i32.load8_u
            local.set $__tmp_i8
            local.get $__tmp_i8
            i32.store8
            local.get $__tmp_i8
            drop
          end
          local.get $__frame
          i32.const 16
          i32.add
          local.tee $__tmp_addr
          i32.load
          local.tee $__tmp_old_i32
          i32.const 1
          i32.add
          local.set $__tmp_i32
          local.get $__tmp_addr
          local.get $__tmp_i32
          i32.store
          local.get $__tmp_old_i32
          drop
          br $do_work_for_loop_2
        end
      end
      local.get $__frame
      i32.const 8
      i32.add
      i32.load
      local.get $__frame
      i32.const 16
      i32.add
      i32.load
      i32.const 1
      i32.mul
      i32.add
      i32.const 0
      local.set $__tmp_i8
      local.get $__tmp_i8
      i32.store8
      local.get $__tmp_i8
      drop
      i32.const 24
      i32.load
      local.get $__parent_frame
      global.set $__frame_ptr
      local.get $__frame
      global.set $__stack_ptr
      return
    end
    local.get $__frame
    i32.const 4
    i32.add
    i32.load
    i32.const 20
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    i32.const 28
    i32.load
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
  )

  ;; function main
  (func $main (result i32)
    (local $__frame i32)
    (local $err i32)
    (local $int_error_value i32)
    (local $string_error_msg i32)
    (local $__tmp_i32 i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 72
    i32.add
    global.set $__stack_ptr
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 1
    local.get $__frame
    i32.const 4
    i32.add
    local.get $__frame
    i32.const 8
    i32.add
    i32.const 64
    call $do_work
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    local.get $__frame
    i32.const 0
    i32.add
    i32.load
    i32.const 28
    i32.load
    i32.eq
    i32.eqz
    i32.eqz
    if
      i32.const 32
      f64.convert_i32_s
      local.get $__frame
      i32.const 4
      i32.add
      i32.load
      f64.convert_i32_s
      f64.const 0
      f64.const 0
      f64.const 0
      f64.const 0
      f64.const 0
      f64.const 0
      call $imp_printf
      drop
    else
      local.get $__frame
      i32.const 0
      i32.add
      i32.load
      i32.const 24
      i32.load
      i32.eq
      i32.eqz
      i32.eqz
      if
        i32.const 56
        f64.convert_i32_s
        local.get $__frame
        i32.const 8
        i32.add
        f64.convert_i32_s
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        call $imp_printf
        drop
      end
    end
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
  )

  (export "do_work" (func $do_work))
  (export "main" (func $main))
  (export "memory" (memory $mem))
  (export "__frame_ptr" (global $__frame_ptr))
  (export "__stack_ptr" (global $__stack_ptr))
  (export "stdin" (global $stdin))
  (export "stdout" (global $stdout))
  (export "stderr" (global $stderr))
)