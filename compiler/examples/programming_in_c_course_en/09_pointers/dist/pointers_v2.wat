;; Generated by MaiaC
;; Template backend: human-readable WAT
;; Style inspired by maiawasm/assembler/tests/fixtures

(module
  (import "env" "toupper" (func $imp_toupper (param i32) (result i32)))
  (import "env" "printf" (func $imp_printf (param f64 f64 f64 f64 f64 f64 f64 f64) (result i32)))
  (import "env" "scanf" (func $imp_scanf (param i32 i32 i32 i32 i32 i32 i32 i32) (result i32)))
  (import "env" "strlen" (func $imp_strlen (param i32) (result i32)))

  (memory $mem 1)

  (table $fn_table 2 funcref)

  ;; global __frame_ptr
  (global $__frame_ptr (mut i32) (i32.const 0))

  ;; global __stack_ptr
  (global $__stack_ptr (mut i32) (i32.const 1024))

  ;; global stdin
  (global $stdin (mut i32) (i32.const 0))

  ;; global stdout
  (global $stdout (mut i32) (i32.const 0))

  ;; global stderr
  (global $stderr (mut i32) (i32.const 0))

  (data (i32.const 16) "Write a word: \00")
  (data (i32.const 32) "%254s\00")
  (data (i32.const 40) "%c\0a\00")
  (data (i32.const 44) "Address pointed to by p: %p\0a\00")
  (data (i32.const 76) "%c\00")
  (data (i32.const 80) "\0a\00")
  (data (i32.const 84) "Text in uppercase: %s\0a\00")

  (elem (table $fn_table) (i32.const 0) func $to_uppercase $main)

  ;; function to_uppercase
  (func $to_uppercase (param $source i32) (param $dest i32)
    (local $__frame i32)
    (local $p i32)
    (local $q i32)
    (local $__tmp_i32 i32)
    (local $__tmp_i8 i32)
    (local $__tmp_addr i32)
    (local $__tmp_old_i32 i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 16
    i32.add
    global.set $__stack_ptr
    local.get $__frame
    i32.const 0
    i32.add
    local.get $source
    i32.store
    local.get $__frame
    i32.const 4
    i32.add
    local.get $dest
    i32.store
    local.get $__frame
    i32.const 8
    i32.add
    local.get $__frame
    i32.const 0
    i32.add
    i32.load
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    local.get $__frame
    i32.const 12
    i32.add
    local.get $__frame
    i32.const 4
    i32.add
    i32.load
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    block $to_uppercase_while_exit_0
      loop $to_uppercase_while_loop_1
        local.get $__frame
        i32.const 8
        i32.add
        i32.load
        i32.load8_u
        i32.eqz
        i32.eqz
        i32.eqz
        br_if $to_uppercase_while_exit_0
        local.get $__frame
        i32.const 12
        i32.add
        i32.load
        local.get $__frame
        i32.const 8
        i32.add
        i32.load
        i32.load8_u
        call $imp_toupper
        local.set $__tmp_i8
        local.get $__tmp_i8
        i32.store8
        local.get $__tmp_i8
        drop
        local.get $__frame
        i32.const 8
        i32.add
        local.tee $__tmp_addr
        i32.load
        local.tee $__tmp_old_i32
        i32.const 1
        i32.add
        local.set $__tmp_i32
        local.get $__tmp_addr
        local.get $__tmp_i32
        i32.store
        local.get $__tmp_old_i32
        drop
        local.get $__frame
        i32.const 12
        i32.add
        local.tee $__tmp_addr
        i32.load
        local.tee $__tmp_old_i32
        i32.const 1
        i32.add
        local.set $__tmp_i32
        local.get $__tmp_addr
        local.get $__tmp_i32
        i32.store
        local.get $__tmp_old_i32
        drop
        br $to_uppercase_while_loop_1
      end
    end
    local.get $__frame
    i32.const 12
    i32.add
    i32.load
    i32.const 0
    local.set $__tmp_i8
    local.get $__tmp_i8
    i32.store8
    local.get $__tmp_i8
    drop
  )

  ;; function main
  (func $main (result i32)
    (local $__frame i32)
    (local $text i32)
    (local $upper_case i32)
    (local $p i32)
    (local $i i32)
    (local $__tmp_i8 i32)
    (local $__tmp_i32 i32)
    (local $__tmp_addr i32)
    (local $__tmp_old_i32 i32)
    (local $__parent_frame i32)
    global.get $__frame_ptr
    local.set $__parent_frame
    global.get $__stack_ptr
    local.set $__frame
    local.get $__frame
    global.set $__frame_ptr
    global.get $__stack_ptr
    i32.const 520
    i32.add
    global.set $__stack_ptr
    i32.const 16
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 32
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 0
    i32.const 0
    i32.const 0
    i32.const 0
    i32.const 0
    i32.const 0
    call $imp_scanf
    drop
    local.get $__frame
    i32.const 0
    i32.add
    i32.const 0
    i32.const 1
    i32.mul
    i32.add
    i32.const 64
    local.set $__tmp_i8
    local.get $__tmp_i8
    i32.store8
    local.get $__tmp_i8
    drop
    local.get $__frame
    i32.const 516
    i32.add
    i32.const 0
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    block $main_for_exit_0
      loop $main_for_loop_2
        local.get $__frame
        i32.const 516
        i32.add
        i32.load
        local.get $__frame
        i32.const 0
        i32.add
        call $imp_strlen
        i32.lt_s
        i32.eqz
        i32.eqz
        i32.eqz
        br_if $main_for_exit_0
        block $main_for_continue_1
          i32.const 40
          f64.convert_i32_s
          local.get $__frame
          i32.const 0
          i32.add
          local.get $__frame
          i32.const 516
          i32.add
          i32.load
          i32.const 1
          i32.mul
          i32.add
          i32.load8_u
          f64.convert_i32_s
          f64.const 0
          f64.const 0
          f64.const 0
          f64.const 0
          f64.const 0
          f64.const 0
          call $imp_printf
          drop
        end
        local.get $__frame
        i32.const 516
        i32.add
        local.tee $__tmp_addr
        i32.load
        local.tee $__tmp_old_i32
        i32.const 1
        i32.add
        local.set $__tmp_i32
        local.get $__tmp_addr
        local.get $__tmp_i32
        i32.store
        local.get $__tmp_old_i32
        drop
        br $main_for_loop_2
      end
    end
    local.get $__frame
    i32.const 512
    i32.add
    local.get $__frame
    i32.const 0
    i32.add
    local.set $__tmp_i32
    local.get $__tmp_i32
    i32.store
    local.get $__tmp_i32
    drop
    i32.const 44
    f64.convert_i32_s
    local.get $__frame
    i32.const 512
    i32.add
    i32.load
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    block $main_while_exit_3
      loop $main_while_loop_4
        local.get $__frame
        i32.const 512
        i32.add
        i32.load
        i32.load8_u
        i32.eqz
        i32.eqz
        i32.eqz
        br_if $main_while_exit_3
        i32.const 76
        f64.convert_i32_s
        local.get $__frame
        i32.const 512
        i32.add
        i32.load
        i32.load8_u
        f64.convert_i32_s
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        f64.const 0
        call $imp_printf
        drop
        local.get $__frame
        i32.const 512
        i32.add
        local.tee $__tmp_addr
        i32.load
        local.tee $__tmp_old_i32
        i32.const 1
        i32.add
        local.set $__tmp_i32
        local.get $__tmp_addr
        local.get $__tmp_i32
        i32.store
        local.get $__tmp_old_i32
        drop
        br $main_while_loop_4
      end
    end
    i32.const 80
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    local.get $__frame
    i32.const 0
    i32.add
    local.get $__frame
    i32.const 256
    i32.add
    call $to_uppercase
    i32.const 84
    f64.convert_i32_s
    local.get $__frame
    i32.const 256
    i32.add
    f64.convert_i32_s
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    f64.const 0
    call $imp_printf
    drop
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
    i32.const 0
    local.get $__parent_frame
    global.set $__frame_ptr
    local.get $__frame
    global.set $__stack_ptr
    return
  )

  (export "to_uppercase" (func $to_uppercase))
  (export "main" (func $main))
  (export "memory" (memory $mem))
  (export "__frame_ptr" (global $__frame_ptr))
  (export "__stack_ptr" (global $__stack_ptr))
  (export "stdin" (global $stdin))
  (export "stdout" (global $stdout))
  (export "stderr" (global $stderr))
)